...
class VehicleModel:
    ...
    def initialize_suspension(self) -> None:
        ...
        # Tires
        self.FL_tire = MF52(tire_name="front_left_tire",
                            file_path=self.tir_file_path)
        self.FR_tire = MF52(tire_name="front_right_tire",
                            file_path=self.tir_file_path)
        self.RL_tire = MF52(tire_name="rear_left_tire",
                            file_path=self.tir_file_path)
        self.RR_tire = MF52(tire_name="rear_right_tire",
                            file_path=self.tir_file_path)
        ...
        self.FL_weight = self.total_mass * \
            self.masses["CGBX"] * self.masses["CGBY"] * self.environment["G"]
        self.FR_weight = self.total_mass * \
            self.masses["CGBX"] * (1 - self.masses["CGBY"]
                                   ) * self.environment["G"]
        self.RL_weight = self.total_mass * \
            (1 - self.masses["CGBX"]) * \
            self.masses["CGBY"] * self.environment["G"]
        self.RR_weight = self.total_mass * \
            (1 - self.masses["CGBX"]) * \
            (1 - self.masses["CGBY"]) * self.environment["G"]
        ...
        self.suspension = SuspensionModel(
            ...
            FR_outboard_points=[[p[0], -1 * p[1], p[2]]
                                for p in hdpts["FLOP"]],
            FR_bellcrank_params=[[p[0], -1 * p[1], p[2]]
                                 for p in hdpts["FLBC"]],
            FR_upper=hdpts["FL_Up"],
            FR_contact_patch=[hdpts["FLCP"][0], -1 *
                              hdpts["FLCP"][1], hdpts["FLCP"][2]],
            ...
            RR_outboard_points=[[p[0], -1 * p[1], p[2]]
                                for p in hdpts["RLOP"]],
            RR_bellcrank_params=[[p[0], -1 * p[1], p[2]]
                                 for p in hdpts["RLBC"]],
            RR_upper=hdpts["RL_Up"],
            RR_contact_patch=[hdpts["RLCP"][0], -1 *
                              hdpts["RLCP"][1], hdpts["RLCP"][2]],
        ...
        self.suspension.generate_kin_plots(steer_sweep=np.linspace(-1.5, 1.5, 26) * 0.0254,
                                           heave_sweep=np.linspace(-1.5,
                                                                   1.5, 26) * 0.0254,
                                           pitch_sweep=np.linspace(-2,
                                                                   2, 26) * np.pi / 180,
                                           roll_sweep=np.linspace(-2, 2, 26) * np.pi / 180)

        self.suspension.plot_elements(
            plotter=self.suspension_plotter, verbose=True, show_grid=False)

        self.suspension_plotter.add_slider(
            func=self.suspension.roll_slider, title="Roll", bounds=[-2, 2], pos=[[0.75, 0.1], [0.98, 0.1]])
        self.suspension_plotter.add_slider(
            func=self.suspension.pitch_slider, title="Pitch", bounds=[-2, 2], pos=[[0.75, 0.225], [0.98, 0.225]])
        self.suspension_plotter.add_slider(
            func=self.suspension.heave_slider, title="Heave", bounds=[-2, 2], pos=[[0.75, 0.350], [0.98, 0.350]])
        self.suspension_plotter.add_slider(
            func=self.suspension.steer_slider, title="Steer", bounds=[-60, 60], pos=[[0.75, 0.475], [0.98, 0.475]])
